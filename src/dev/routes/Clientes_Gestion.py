...
async def get_all_clients(user: dict = Depends(only_user)):
    ...
    try:
        ...
        return {"status": "success", "data": clients, "user": user.get("name")}
...
async def get_client_by_id(client_id: int, user: dict = Depends(only_user)):
    try:
...
async def update_client(
    client_id: int,
    data: dict,
    user: dict = Depends(require_role("admin", "aux_almacen")),
):
    try:
        if not data:
            raise HTTPException(
...
async def delete_client(
    client_id: int, user: dict = Depends(require_role("admin", "aux_almacen"))
):
    try:
